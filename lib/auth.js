...
async function login(password) {
  ...
  const db = sql();
  // OJO: no interpolar SESSION_HOURS dentro del literal INTERVAL '...' —
  // el driver parametrizado (tagged template) trata cualquier ${} como bind
  // param, y un $N dentro de comillas simples rompe en Postgres. Se
  // multiplica un INTERVAL fijo de 1 hora en vez de armar el string.
  await db`
    INSERT INTO febocar_admin_sesiones (token, expira_at)
    VALUES (${token}, NOW() + INTERVAL '1 hour' * ${SESSION_HOURS})
  `;
  return token;
}
